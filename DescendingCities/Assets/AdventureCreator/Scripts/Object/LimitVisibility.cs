...
namespace AC
{
	...
	public class LimitVisibility : MonoBehaviour
	{
		...
		protected GUITexture[] childGUITextures;
		#endif
		#if ALLOW_VIDEO
		...
		protected void Awake ()
		{
			...
			if (affectChildren)
			{
				...
				#if ALLOW_LEGACY_UI
				childGUITextures = GetComponentsInChildren <GUITexture>();
				#endif
			}
		...
		protected void SetVisibility (bool state)
		{
			...
			if (affectChildren)
			{
				...
				#if ALLOW_LEGACY_UI
				foreach (GUITexture child in childGUITextures)
				{
					child.enabled = state;
				}
				#endif
