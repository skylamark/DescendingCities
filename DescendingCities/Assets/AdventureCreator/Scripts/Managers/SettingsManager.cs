	...
	public class SettingsManager : ScriptableObject
	{
		...
		private void ShowSaveGameSettings ()
		{
			...
			if (showSave)
			{
				...
				{
					useJsonSerialization = CustomGUILayout.ToggleLeft ("Save in Json format? (Experiemental)", useJsonSerialization, "AC.KickStarter.settingsManager.useJsonSerialization", "If True, then Json serialization will be used instead");
		...
		private void ShowInventorySettings ()
		{
			...
			if (showInventory)
			{
				...
				canReorderItems = CustomGUILayout.ToggleLeft ("Items can be re-ordered in Menu?", canReorderItems, "AC.KickStarter.settingsManager.canReorderItems", "If True, then inventory items can be re-ordered in an InventoryBox menu element by the player");
		...
		private void ShowOptionsSettings ()
		{
			...
			if (showOptions)
			{
				...
				defaultSpeechVolume = optionsData.speechVolume = CustomGUILayout.Slider ("Speech volume:", optionsData.speechVolume, 0f, 1f, "AC.KickStarter.settingsManager.defaultSpeechVolume", "The game's default speech audio volume");
				defaultMusicVolume = optionsData.musicVolume = CustomGUILayout.Slider ("Music volume:", optionsData.musicVolume, 0f, 1f, "AC.KickStarter.settingsManager.defaultMusicVolume", "The game's default music audio volume");
				defaultSfxVolume = optionsData.sfxVolume = CustomGUILayout.Slider ("SFX volume:", optionsData.sfxVolume, 0f, 1f, "AC.KickStarter.settingsManager.defaultSfxVolume", "The game's default SFX audio volume");
				defaultShowSubtitles = optionsData.showSubtitles = CustomGUILayout.Toggle ("Show subtitles?", optionsData.showSubtitles, "AC.KickStarter.settingsManager.defaultShowSubtitles", "The game's default subtitles state");
				defaultLanguage = optionsData.language = CustomGUILayout.IntField ("Language:", optionsData.language, "AC.KickStarter.settingsManager.defaultLanguage", "The game's default language index, where 0 is the game's original language");
				if (KickStarter.speechManager != null && KickStarter.speechManager.separateVoiceAndTextLanguages)
				{
					defaultVoiceLanguage = optionsData.voiceLanguage = CustomGUILayout.IntField ("Speech audio language:", optionsData.voiceLanguage, "AC.KickStarter.settingsManager.defaultVoiceLanguage", "The game's default voice language index");
				}
