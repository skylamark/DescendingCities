	...
	public class MenuInventoryBox : MenuElement
	{
		...
		public override void ShowGUI (Menu menu)
		{
			...
			else if (inventoryBoxType == AC_InventoryBoxType.Container)
			{
				isClickable = true;
				maxSlots = CustomGUILayout.IntSlider ("Max number of slots:", maxSlots, 1, 30, apiPrefix + ".maxSlots", "The maximum number of inventory items that can be shown at once");
				selectItemsAfterTaking = CustomGUILayout.Toggle ("Select item after taking?", selectItemsAfterTaking, apiPrefix + ".selectItemsAfterTaking", "If True, then items will be selected automatically when they are removed from the Container");
			}
			else
			{
				isClickable = true;
			...
			hoverSoundOverEmptySlots = CustomGUILayout.Toggle ("Hover sound over empty slots?", hoverSoundOverEmptySlots, apiPrefix + ".hoverSoundOverEmptySlots", "If True, then the hover sound will play even when over an empty slot");
		...
		public override void ProcessClick (AC.Menu _menu, int _slot, MouseState _mouseState)
		{
			...
			if (inventoryBoxType == AC_InventoryBoxType.CollectedDocuments)
			{
				if (autoOpenDocument)
				{
					Document document = GetDocument (_slot + offset);
					KickStarter.runtimeDocuments.OpenDocument (document);
				}
			}
			else if (inventoryBoxType == AC_InventoryBoxType.CustomScript)
			{
