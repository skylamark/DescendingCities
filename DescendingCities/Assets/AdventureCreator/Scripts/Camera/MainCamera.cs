...
namespace AC
{
	...
	public class MainCamera : MonoBehaviour
	{
		...
		public CameraSplitOrientation splitOrientation;
			...
			if (attachedCamera && attachedCamera.Camera)
			{
				...
				if (changedOrientation && !SceneSettings.IsUnity2D () && KickStarter.stateHandler.IsInGameplay () && KickStarter.settingsManager.movementMethod == MovementMethod.Direct && KickStarter.settingsManager.directMovementType == DirectMovementType.RelativeToCamera && /*KickStarter.settingsManager.inputMethod != InputMethod.TouchScreen &&*/ KickStarter.playerInput != null)
				{
						...
						(KickStarter.player.GetPath () == null || !KickStarter.player.IsLockedToPath ()))
					{
						...
						Debug.LogWarning ("Lock snap enabled!");
					}
				}
			}
		}
		...
		public Rect GetSplitScreenRect (bool isMainCamera)
		{
			if (splitOrientation == CameraSplitOrientation.Overlay)
			{
				...
				if (borderOrientation == MenuOrientation.Vertical)
				{
					return new Rect (borderWidth, 0f, 1f - (2*borderWidth), 1f);
				}
				return new Rect (0f, borderWidth, 1f, 1f - (2*borderWidth));
		...
		protected IEnumerator ExitSceneWithOverlay ()
		{
			...
			if (QualitySettings.activeColorSpace == ColorSpace.Linear)
			{
				int _width = screenTex.width;
				int _height = screenTex.height;
				...
				for (int y = 0; y < _height; y++)
				{
					for (int x = 0; x < _width; x++)
					{
						...
						screenTex.SetPixel (x, y, color.gamma);
						#else
		...
		protected bool SetAspectRatio ()
		{
			...
			if (currentAspectRatio > KickStarter.settingsManager.wantedAspectRatio)
			{
				...
				borderRect1 = new Rect (0, 0, borderWidth * screenSize.x, screenSize.y);
				borderRect2 = new Rect (screenSize.x * (1f - borderWidth), 0f, screenSize.x * borderWidth, screenSize.y);
			}
			else
			{
				// Letterbox
				...
				borderRect1 = new Rect (0, 0, screenSize.x, borderWidth * screenSize.y);
				borderRect2 = new Rect (0, screenSize.y * (1f - borderWidth), screenSize.x, screenSize.y * borderWidth);
			}
				...
				if (KickStarter.playerCursor.LimitCursorToMenu.IsUnityUI ())
				{
					if (KickStarter.playerCursor.LimitCursorToMenu.RuntimeCanvas != null)
					{
						if (KickStarter.playerCursor.LimitCursorToMenu.RuntimeCanvas.renderMode == RenderMode.ScreenSpaceOverlay)
						{
							...
							Rect menuRect = new Rect (rectTransform.position.x, Screen.height - rectTransform.position.y, size.x, size.y);
							...
							point.y = Mathf.Clamp (point.y, Screen.height - menuRect.y - menuRect.height, Screen.height - menuRect.y);
						}
						else
						{
